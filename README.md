...
The CUT program that we prepared indeed contains bugs. You do not need to locate the bugs in assignment 1. We will work on them in assignment 2.

### Platform

Assignment 1 requires the use of both Eclipse and command line with Java 11 (SE).
...
A short tutorial about Randoop is avaiable [here](./tutorials/randoop.md), including how to use Randoop to generate unit tests, and execute them using Eclipse.
...
EclEmma is a bundled Eclipse plugin to measure the code coverage. A tutorial is available [here](./tutorials/eclipse_eclemma.md).
...
- When instrumenting the CUT, you are required to skip the `JIdentityStmt`. Please refer to [FAQ Q3](./Assignment1_FAQ.md#3-why-we-got-the-exception-javalangruntimeexception-param-assignment-statements-should-precede-all-non-identity-statements-) on why do we need to skip the `JIdentityStmt` and the hint on how to skip it.
- To validate the correctness of your statement coverage report, you can crosscheck it with the report generated by EclEmma. The coverage reported by your tool should be close to that reported by EclEmma for most of classes in the CUT.
...
#### Notes:
- The implementation is similar to that of task 2. While we insert code for each line in task 2, we insert code for each branch in task 3.
...
#### Notes:
The implementation of this bonus task is similar to that of task 2 except that you need to obtain the line number of Jimple code. Please refer to [FAQ Q9](./Assignment1_FAQ.md#9-how-to-preserve-the-line-number-in-soot) on how to obtain the line number.
